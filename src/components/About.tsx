    ...
    <div id='about' className='bg-gray-50 py-10 text-center'>
      <div className='w-4/5 mx-auto flex flex-col gap-5 items-center'>
        <div className='flex flex-col'>
          <div>
            <h1 ref={titleRef} className={`text-4xl font-bold ${titleIsVisible ? "animate-in slide-in-from-bottom duration-500 fade-in" : ""}`}>About Me</h1>
          </div>
          
          <div className='mt-5 flex flex-col gap-10 justify-center xl:flex-row xl:gap-20'>
            <div ref={contentRef} className={`w-4/5 mx-auto text-center xl:w-1/3 ${contentIsVisible ? "animate-in slide-in-from-bottom duration-500 fade-in" : ""}`}>
            ...
            <div ref={terminalRef} className={`min-h-unit-7xl max-h-unit-7xl overflow-x-hidden overflow-y-hidden hover:cursor-text bg-[#272B36] rounded-lg ${terminalIsVisible ? "animate-in slide-in-from-bottom duration-500 fade-in" : ""}`}>
              <Terminal />
            </div>
          </div>
        </div>
