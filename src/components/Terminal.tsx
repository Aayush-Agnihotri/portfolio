...
export default function Terminal() {
  ...
  const commands = {
    ...
    website: (
      <div className="xl:max-w-xl">
        <p>
          &gt; This website was created using Next.js 14 and Tailwind CSS. To
          see the GitHub repository, click{" "}
          <a
    ...
    awards: (
      <div className="xl:max-w-xl">
        <p>
          &gt; <span className="text-orange-400">JPMorgan Chase</span> First
          Place Winner at Technology for Social Good 2021
        </p>
        <p>
          &gt; <span className="text-orange-400">TSA</span> First Place Winner
          State Software Development Competition 2021
        </p>
        <p>
          &gt; <span className="text-orange-400">TSA</span> Third Place Winner
          National Software Development Competition 2021
        </p>
        <p>
          &gt; <span className="text-orange-400">JPMorgan Chase</span> First
          Place Winner at Technology for Social Good 2018
        </p>
      </div>
    ),
    skills: (
      <div className="xl:max-w-xl">
        {TechnicalData.map((data, i) => (
          <p key={i}>
            &gt; <span className="text-orange-400">{data.type}:</span>{" "}
    ...
    work: (
      <div className="xl:max-w-xl">
        {WorkData.map((data, i) => (
          <p key={i}>
            &gt; <span className="text-orange-400">{data.company}:</span>{" "}
            {data.role} ({data.date})
          </p>
        ))}
      </div>
